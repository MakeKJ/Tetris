...
def speed():
    ...
    return int(STARTING_SPEED * 0.9**multiplier)
...
def game_over():
    ...
    canvas.create_text(canvas.winfo_width()/2, canvas.winfo_height()*4/7,
...
rows, cols = 20, 10
bin_matrix = [([0] * cols) for i in range(rows)]

#  Stores drawn pictures of the squares.
square_matrix = [([0] * cols) for i in range(rows)]
...
window.bind('<Up>', lambda event: change_orientation())
window.bind('<Left>', lambda event: move_to(1))
window.bind('<Right>', lambda event: move_to(2))
window.bind('<Down>', lambda event: fastforward())
window.bind('<space>', lambda event: skip_turn())
